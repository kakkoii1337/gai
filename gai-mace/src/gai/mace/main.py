...
from gai.mace.user.api.dependencies import get_settings
settings = get_settings()

# Report arguments
console.print(f"[yellow]connection={settings.NATS}[/yellow]")
console.print(f"[yellow]ttt={settings.TTT}[/yellow]")
console.print(f"[yellow]rag={settings.RAG}[/yellow]")
console.print(f"[yellow]tti={settings.TTI}[/yellow]")

# Run Node
async def run_node():
    # import persona
    import_dir=f"~/.gai/persona/nodes/{settings.PERSONA}"
...
async def run_user(app):
    ...
    #     import_dir = "~/.gai/persona/00000000-0000-0000-0000-000000000000"
    #     import_dir=os.path.expanduser(import_dir)
    #     if not os.path.exists(import_dir):
    #         console.print(f"[magenta]Custom persona is not created yet. Abort loading persona thread.[/magenta]")
    #         return
